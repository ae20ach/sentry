...
| `tests/sentry/digests/backends/test_redis.py::RedisBackendTestCase::test_large_digest`                                              | `assert 5587 == 8192` — partial records after `flushdb()` mid-loop                                              | 8192 sequential Redis `add()` calls; a concurrent `flushdb()` clears some entries mid-loop. Fix: use a unique timeline key per test (e.g., `f"timeline:{uuid4().hex}"`) so `flushdb()` doesn't affect in-progress data.                                                                                                                              |
