...
| `tests/snuba/tasks/test_unmerge.py::UnmergeTestCase::test_unmerge`                                              | `assert 16 == 6` in TSDB `assert_series_contains` — source.id still has merged-count (16) instead of post-unmerge (6). Or previously `times_seen=19`. | The 60s poll waits for ClickHouse event count to stabilize, but Snuba's TSDB/Redis data for the merge (set by `tsdb.backend.merge`) is cleared by concurrent `flushdb()` then not correctly rebuilt by `repair_tsdb_data`. Root cause: TSDB data is Redis-based and non-transactional; both merge's `tsdb.backend.merge` and unmerge's `truncate_denormalizations`/`repair_tsdb_data` are vulnerable to concurrent `flushdb()`. Needs investigation into `RedisSnubaTSDB.get_range` read path vs write path under parallel load. |
