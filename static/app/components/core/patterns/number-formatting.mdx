...
In order to nicely format numbers, we have a few tools at our disposal:
...
- **Minimum Obfuscation**. In some contexts, very small numbers are not meaningful. e.g., a `"rate"` of `0.000012` per minute is not useful to show in a table. For applicable types (e.g., `"rate"`) in applicable contexts (e.g., table cells), **Sentry may obfuscate values &lt;0.0001** and replace them with "&lt;0.0001". This might not be appropriate. Consider replacing this with scientific notation.
...
- **Missing Values**. Display of missing values depends on the context. Textual display like a table uses a greyed-out em-dash.

## UI Context

TODO: Describe the various contexts, and the decisions we make there. e.g., how many decimal places in tables, how many significant figures in tooltips, Y-axes, and so on.
